...
.. csv-table::
   ...
   分析ジョブID,文字列,分析ジョブを一意に示すID,"- 32文字の英数字
   - 以下の文字からなる
   ...
   最小値,実数,分析に使用したデータの最小値,- 半角数字
   最大値,実数,分析に使用したデータの最大値,- 半角数字
...
- 画面左部にパラメーター入力フォームが表示される
  ...
  - 期間入力フォームを選択するとカレンダーが表示され，開始日時と終了日時を指定できる
    ...
    - 開始日時には最も古いレートの日時が設定されている
    - 終了日時には最も新しいレートの日時が設定されている
...
- フォームの下部に現在の定期予測設定がペアごとに表示される

  - テーブルの各行の色は状態によって以下の通りに設定される
...
- 設定ボタンを押下すると有効にする設定を行い，ジョブ登録フォームを表示する
...
- 設定ボタンを押下すると無効にする設定を行い，ジョブ登録フォームを表示する
