...
operationパラメーターがcreateの場合は4〜9を実行する
...
6. 素性をDBに登録するメソッドを実行する
7. レース，エントリー，競走馬，騎手情報から素性を抽出する
8. それ以外の素性を抽出する
...
operationパラメーターがupdateの場合は10〜15を実行する

10. 既存の素性を更新するメソッドを実行する
11. 対象のIDの素性情報を検索する

取得した素性だけ12〜15を繰り返す

12. 素性を更新するメソッドを実行する
13. レース，エントリー，競走馬，騎手情報から素性を抽出する
14. それ以外の素性を抽出する

素性の作成に成功した場合は15を実行する
