...
.. csv-table::
   ...
   data_source,string,分析に使用する学習データの情報源,"- 以下のいずれか
...
.. csv-table::
   ...
   race_id,string,分析に利用したレースのID, :ref:`den-ext-res-race` 参照
...
- 学習データでは以下から選択する
  ...
  - ランダム: システムがランダムに学習データを選択する

    - 選択するとセレクトボックスの下にする学習データ数を入力するテキストボックスが表示される

  - ファイル: レースIDが改行区切りで記載されたファイルを指定する

    - 選択するとセレクトボックスの下にファイル入力フォームが表示される

  - デフォルトでは ランダム が選択されている
