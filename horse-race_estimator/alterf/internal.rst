...
9. 分析ジョブ情報の状態を進行中にする
10. 分析スクリプトを実行する
11. スクリプトの出力ファイルをチェックする
12. 分析結果情報を出力ファイルからインポートする
13. 重要度情報をファイルから読み込む
14. 重要度情報をDBに登録する
15. 決定木情報をDBに登録する
...
16. 決定木の構成をファイルからインポートする
17. ノード情報をファイルから読み込む
18. ノード情報をDBに登録する

19. 分析ジョブ情報の素性の数を更新する
20. 分析ジョブIDをファイルに出力する
21. モデルやメタデータなど予測に必要なファイルをまとめてzip化する
22. 素性の情報をまとめてzip化する
23. ダウンロード用ファイルを作成する
24. 分析結果をメールで通知する
25. 分析ジョブ情報の状態を完了にする
...
8. GET /analyses を実行する
9. 分析ジョブ一覧画面を更新する

10. 利用者が結果表示ボタンを押下する
11. 分析ジョブ情報を取得する
12. 1ヶ月以上前に作成された一時ファイルを削除する
13. 分析ジョブIDから分析結果情報を取得する
...
14. 14〜16を実行して分析ジョブ情報を評価ジョブ情報に設定する
15. ファイルから分析ジョブIDを読み込む
16. 分析ジョブIDから分析ジョブ情報を取得する
17. 評価ジョブ情報を更新して分析ジョブ情報を紐づける
18. 19〜25を実行して評価データ情報を作成する

:ref:`alt-ext-ui-evaluation` でランダムを選択した場合は19を実行する

19. データベースからレースIDをランダムに取得する

:ref:`alt-ext-ui-evaluation` で Top20 を選択した場合は20, 21を実行する

20. HTTPクライアントを作成する
21. 外部サイトからレースIDを20件取得する

:ref:`alt-ext-ui-evaluation` で Top20，ランダム以外を選択した場合は22を実行する

22. ファイルからレースIDを取得する

取得したレースIDごとに23〜25を繰り返す

23. レースIDからレース情報を取得する
24. レースIDから正解の素性情報を取得する
25. 評価データ情報をデータベースに保存する
...
28. 抽出した素性をYAMLファイルに出力する
29. モデルを予測するスクリプトを実行する

30. 31, 32を実行して予測結果情報を作成する
31. 予測結果が書かれたファイルを読み込む
32. 予測結果情報をデータベースに保存する
