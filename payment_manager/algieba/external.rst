...
.. csv-table::
   ...
   "ID", "自然数(integer)", "収支情報にユニークに設定された値", "- 半角数字
   - システムによって自動的に付与される
   - 変更不可"
   "種類", "文字列(string)", "収入, 支出を表す文字列", "- 以下のいずれか

     - income
     - expense"
   "日付", "文字列(string)", "所持金の増減があった日時", "- yyyy-mm-dd の形式"
   "内容", "文字列(string)", "所持金の増減があった理由など", "- 任意の文字列"
   "カテゴリ", ":ref:`alg-ext-res-category` の配列(array[:ref:`alg-ext-res-category` ])", "カテゴリリソースの名前",
...
.. csv-table::
   ...
   "ID", "自然数(integer)", "カテゴリ情報にユニークに設定された値", "- 半角数字
   - システムによって自動的に付与される
   - 変更不可"
...
.. csv-table::
   ...
   "ID", "自然数(integer)", "辞書情報にユニークに設定された値", "- 半角数字
   - システムによって自動的に付与される
   - 変更不可"
   "内容", "文字列(string)", "所持金の増減があった理由など", "- 任意の文字列"
   "条件", "文字列(string)", "内容とカテゴリを紐付けるかどうかを決める条件", "- 以下のいずれか

     - equal: 完全一致する内容にカテゴリを設定する
     - include: 部分一致する内容にカテゴリを設定する"
   "カテゴリ", ":ref:`alg-ext-res-category` の配列(array[:ref:`alg-ext-res-category` ])", "カテゴリリソースの名前",
